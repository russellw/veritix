...
- the **GNU Affero General Public Licence, version 3 or later** (AGPL-3.0-or-later),
  the full text of which is in [`LICENSE`](LICENSE); **or**
- a **commercial licence** from the copyright holder, on terms that do not
...
otherwise in its own header. Nothing here is a dual-licence trick where the
open version is crippled: the AGPL build is the whole product. The commercial
licence sells different *terms*, not different code.
...
your own organisation — on a laptop, on a build server, in your own cloud —
whether or not you have modified it. That is the case Veritix was built for,
and it is the case the licence leaves alone.

**You probably want a commercial licence if you**

- ship Veritix, or code derived from it, inside a product you distribute under
  terms of your own;
- run a modified Veritix as a service that people outside your organisation
...
- need the things a public licence cannot give: a warranty, an indemnity, a
...
licence and it does not modify it — [`LICENSE`](LICENSE) governs.
...
licence — MIT, BSD-3-Clause, or Apache-2.0 — which is deliberate: a copyleft
dependency would be a term the commercial licence could not deliver. The
...
Their terms travel with the binary and are unaffected by which Veritix licence
you hold.
...
Contributions are accepted under the Contributor Licence Agreement in
[`CLA.md`](CLA.md), which is what makes the dual licence possible: the
